...
static int aoe_maxioc = 8192;	/* default meant to avoid most soft lockups */
...
skb_pool_get(struct aoedev *d)
{
	...
	if (d->nskbpool < NSKBPOOLMAX && (skb = new_skb(ETH_ZLEN))) {
		d->nskbpool++;
		return skb;
	}
	return NULL;
...
newtframe(struct aoedev *d, struct aoetgt *t)
{
	...
	if (list_empty(&t->ffree)) {
		if (t->falloc >= NSKBPOOLMAX*2)
			return NULL;
		f = aoe_kcalloc(1, sizeof *f, GFP_ATOMIC);
		if (f == NULL)
			return NULL;
		t->falloc++;
		f->t = t;
...
resend(struct aoedev *d, struct frame *f)
{
	...
	if (ifrotate(t) == NULL) {
		...
		printk(KERN_INFO "aoe: resend: no interfaces to rotate to.");
		ktcomplete(f, NULL);
		return;
	}
	h = (struct aoe_hdr *) skb_mac_header(skb);
...
rexmit_timer(ulong vp)
{
	...
	while (!list_empty(&flist)) {
		...
		t->lost += 1;	/* (decremented on unexpected response) */
		if (t->maxout != 1) {
...
nextbuf(struct aoedev *d)
{
	...
	if (buf == NULL) {
		printk(KERN_ERR "aoe: nextbuf: unable to mempool_alloc!\n");
		return 0;
	}
	bio = d->ip.nxbio;
	bufinit(buf, rq, bio);
...
static void
calc_rttavg(struct aoedev *d, struct aoe_hdr *h,
	    struct frame *f,	/* for tracing RTTs */
	    struct aoetgt *t, int rtt)
{
	register long n;

	n = rtt;
	aoedbg_rtt(d, h, f, rtt);
...
aoecmd_ata_rsp(struct sk_buff *skb)
{
	...
	f = getframe(d, n);
	if (f) {
		calc_rttavg(d, h, f, f->t, tsince_hr(f));
		f->t->nout--;
		if (f->flags & FFL_PROBE)
			f->t->nout_probes--;
	} else {
		...
		if (f) {
			calc_rttavg(d, h, f, NULL, tsince_hr(f));
		} else {
			calc_rttavg(d, h, NULL, NULL, tsince(n));
			t = gettgt(d, h->src);
			if (t)
				t->lost--; /* packet wasn't lost, just late */
			spin_unlock_irqrestore(&d->lock, flags);
			aoedev_put(d);
			snprintf(ebuf, sizeof ebuf,
				"%15s e%d.%d    tag=%08x@%08lx s=%012llx d=%012llx\n",
				"unexpected rsp",
				be16_to_cpu(get_unaligned(&h->major)),
				h->minor,
				be32_to_cpu(get_unaligned(&h->tag)),
				jiffies,
				mac_addr(h->src),
				mac_addr(h->dst));
			aoechr_error(ebuf);
...
grow_targets(struct aoedev *d)
{
	...
	oldn = d->ntargets;
	newn = oldn * 2;
	tt = aoe_kcalloc(newn, sizeof(*d->targets), GFP_ATOMIC);
	if (!tt)
		return NULL;
...
addtgt(struct aoedev *d, char *addr, ulong nframes)
{
	...
	t = aoe_kcalloc(1, sizeof(*t), GFP_ATOMIC);
	if (!t)
		goto nomem;
	t->nframes = nframes;
	t->d = d;
...
aoecmd_cfg_rsp(struct sk_buff *skb)
{
	...
	if (h->minor == 0xff) {
		printk(KERN_INFO "aoe: e%ld.%d: broadcast slot number invalid\n",
			aoemajor, (int) h->minor);
		return;
	}
	...
	if (d == NULL) {
		printk(KERN_INFO "aoe: device allocation failure\n");
		return;
	}
...
aoe_flush_iocq_by_index(int id)
{
	...
	spin_lock_irqsave(&iocq[id].lock, flags);
	list_splice(&iocq[id].head, &flist);
	INIT_LIST_HEAD(&iocq[id].head);
	spin_unlock_irqrestore(&iocq[id].lock, flags);
	while (!list_empty(&flist)) {
		pos = flist.next;
		list_del(pos);
...
aoecmd_init(void)
{
	...
	*/
	...
	iocq = aoe_kcalloc(ncpus, sizeof(struct iocq_ktio), GFP_KERNEL);
	if (!iocq)
		return -ENOMEM;

	kts = aoe_kcalloc(ncpus, sizeof(struct ktstate), GFP_KERNEL);
	if (!kts) {
		ret = -ENOMEM;
		goto kts_fail;
	}

	ktiowq = aoe_kcalloc(ncpus, sizeof(wait_queue_head_t), GFP_KERNEL);
	if (!ktiowq) {
		ret = -ENOMEM;
		goto ktiowq_fail;
	}
